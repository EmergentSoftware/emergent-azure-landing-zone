# Customer Portal Workload - Azure Static Web App

This workload deploys the customer-facing portal as an Azure Static Web App with network isolation and private endpoint support.
...
- **Network isolation**: Dedicated VNet per environment, isolated from admin portal
...
```
Resource Group (acme-rg-portals-customer-app-{env}-{region}-{unique})
├── Static Web App (acme-swa-customer-{env}-{region}-{unique})
│   ├── SKU: Free (dev) | Standard (prod with private endpoint)
│   ├── System-assigned Managed Identity
│   └── Tags: Environment, Purpose, Workload
│
└── (Optional) Private Endpoint
    ├── Connects to: Customer Dev VNet (10.110.0.0/16)
    ├── Subnet: Private Endpoints (10.110.2.0/24)
...
1. **Landing Zone Network**: `02-landing-zones/workloads/portals-customer-dev` deployed
...
2. **Connectivity Hub**: `02-landing-zones/connectivity` deployed
   ...
   - VNet Peering: hub ↔ customer-dev spoke (required for private DNS resolution)

3. **Terraform**: >= 1.12.0
4. **Subscription**: portals-customer-dev (9a877ddf-9796-43a8-a557-f6af1df195bf)

### Network Configuration

Before deploying, ensure VNet peering is configured between hub and customer-dev spoke:
...
```hcl
private_endpoint_subnet_id = "/subscriptions/9a877ddf-9796-43a8-a557-f6af1df195bf/resourceGroups/.../subnets/acme-snet-customer-privateendpoints-dev-eus2"
```

### 2. Initialize Terraform

```powershell
cd ../../../03-workloads/portals/customer-portal
...
2. **Verify Private DNS Resolution**:
   ```powershell
   # From a VM in the customer VNet
   nslookup <your-swa-name>.azurestaticapps.net
   # Should resolve to private IP 10.110.2.x
   ```

### VNet Integration (Optional)

If the Static Web App needs to call backend APIs in the customer VNet:
...
- **Customer Dev**: Uses dedicated VNet (10.110.0.0/16) isolated from admin portal
...
```hcl
environment         = "prod"
subscription_id     = "b13c8883-9bf3-4ebb-af9c-7ebfcc8e9a5a"  # Customer-prod subscription
static_web_app_sku  = "Standard"
enable_private_endpoint = true
private_endpoint_subnet_id = "<subnet-from-portals-customer-prod-landing-zone>"
```
...
terraform init -backend-config="key=tfstate-customer-portal-prod" -reconfigure
...
az staticwebapp list --query "[?contains(name, 'customer')].{Name:name, ResourceGroup:resourceGroup, Hostname:defaultHostname, SKU:sku.name}" -o table

# Check private endpoint (if enabled)
az network private-endpoint list --query "[?contains(name, 'customer')].{Name:name, ResourceGroup:resourceGroup, PrivateIP:customDnsConfigs[0].ipAddresses[0]}" -o table
...
Write-Host "Customer Portal: https://$hostname"

# Test from Azure (if private endpoint enabled)
# curl https://$hostname from a VM in the customer VNet
```

## Next Steps

1. **Configure CI/CD**: Use the deployment token with GitHub Actions or Azure DevOps
2. **Deploy Application**: Push your customer portal frontend code to trigger deployment
...
```powershell
# Verify VNet peering
az network vnet peering show --resource-group <hub-rg> --vnet-name vnet-hub-prod-eus2 --name hub-to-customer-dev
...
- [02-landing-zones/workloads/portals-customer-dev/README.md](../../02-landing-zones/workloads/portals-customer-dev/README.md) - Network landing zone
